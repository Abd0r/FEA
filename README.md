...
The C++ code simulates 100,000 Fusion Cores (6.55e9 blocks) with vector ADD, matrix multiply, CAM search, sparse inference, and Slingshot messaging. The output is saved to M+FEA_Chip_Sim_Output.md.
...
Thermal BER                     8e-7 per operation                   sim_ber.py
32-bit ADD (1e7 parallel words) 3.49 ns, 2.87 POPS                   sim_arithmetic.py
Stochastic FP (L=1024)          1.65% rel. error, 3200x speedup      sim_stochfp.py
Slingshot per-hop latency       1.96 ns, BER < 3e-13                 sim_slingshot.py
